...
export const NavBar = () => {
  ...
  useEffect(() => {
    const fetchTourCategories = async () => {
      try {
        const response = await tourCategoryService.getAllTourCategories();
        const tourCategories = response.data;
        setNavigation((prev) => {
          ...
          return prev.map((item) =>
            item.id === "tour" ? { ...item, dropdownItems } : item
          );
        });
        console.log("Updated navigation data:", navigation);
      } catch (error) {
  ...
  useEffect(() => {
    console.log("Navigation đã được cập nhật:", navigation);
  }, [navigation]);
