...
const TabPrice: React.FC<TabPriceProps> = ({ tourId }) => {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Đang tải bảng giá...</div>;
  if (error) return <div className="p-8 text-center text-red-500">{error}</div>;
  if (!prices.length) return <div className="p-8 text-center text-gray-500">Chưa có bảng giá cho tour này.</div>;
        ...
        <div className="border-2 border-blue-800 rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {prices.map((row, idx) => (
                  <tr key={row.id} className="border-b border-blue-800 last:border-b-0">
                    <td className="py-3 px-4 text-center">{idx + 1}</td>
                    <td className="py-3 px-4 font-bold text-lg text-gray-800 text-center">{row.adult_price.toLocaleString()} VNĐ</td>
                    <td className="py-3 px-4 font-bold text-lg text-gray-800 text-center">{row.kid_price.toLocaleString()} VNĐ</td>
                    <td className="py-3 px-4 text-gray-700 text-center">{row.note}</td>
                    <td className="py-3 px-4 text-center">
                      <button className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-5 py-2 rounded-full transition-all">ĐẶT NGAY</button>
                    </td>
                  </tr>
                ))}
              </tbody>
