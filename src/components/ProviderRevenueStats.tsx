...
import { Calendar, TrendingUp, DollarSign, Users, CheckCircle, XCircle, Clock, BarChart3, ChevronDown, Filter, RefreshCw, PieChart, CalendarDays, MapPin } from 'lucide-react';
import { 
...
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, Legend, PieChart as RechartsPieChart, Pie, Cell } from 'recharts';

interface ProviderRevenueStatsProps {
  providerId: number;
}

export const ProviderRevenueStats: React.FC<ProviderRevenueStatsProps> = ({ providerId }) => {
  ...
  const fetchMonthlyData = async () => {
    ...
    try {
      const response = await getProviderMonthlyRevenue(providerId, selectedYear);
    ...
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
            <div>
              ...
              <CardDescription>
                Từ {startDate && format(startDate, 'dd/MM/yyyy', { locale: vi })} 
              ...
              <PopoverContent className="w-auto p-0" align="end">
                <div className="p-4">
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm font-medium text-muted-foreground mb-2 block">Từ ngày:</label>
                      <CalendarComponent
                        mode="single"
                        selected={startDate}
                    ...
                    <Separator />
                    <div>
                      <label className="text-sm font-medium text-muted-foreground mb-2 block">Đến ngày:</label>
                      <CalendarComponent
                        mode="single"
                        selected={endDate}
                        onSelect={setEndDate}
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
        ...
        <CardContent>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p className="flex items-start">
              ...
              Trong khoảng thời gian từ {startDate && format(startDate, 'dd/MM/yyyy', { locale: vi })} 
              {' '}đến {endDate && format(endDate, 'dd/MM/yyyy', { locale: vi })}, 
              bạn đã có <Badge variant="outline" className="mx-1">{formatNumber(data.total_bookings)}</Badge> đặt tour với tổng doanh thu <Badge variant="default" className="mx-1">{formatCurrency(data.total_revenue)}</Badge>.
            </p>
            <p className="flex items-start">
              <span className="mr-2">•</span>
              Trung bình mỗi đặt tour có giá trị <Badge variant="secondary" className="mx-1">{formatCurrency(data.average_booking_value)}</Badge>.
            </p>
            <p className="flex items-start">
              <span className="mr-2">•</span>
              Tỷ lệ thành công đạt <Badge variant="default" className="mx-1 bg-green-100 text-green-800 hover:bg-green-100">{data.success_rate}%</Badge> với <Badge variant="outline" className="mx-1">{formatNumber(data.success_bookings)}</Badge> đặt tour thành công.
            </p>
            <p className="flex items-start">
              <span className="mr-2">•</span>
              Hiện có <Badge variant="outline" className="mx-1">{formatNumber(data.active_tours)}/{formatNumber(data.total_tours)}</Badge> tour đang hoạt động 
              (tỷ lệ hoạt động: <Badge variant="secondary" className="mx-1">{data.tour_activity_rate}%</Badge>).
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
            <div>
              <CardTitle className="text-xl">Biểu đồ doanh thu theo tháng</CardTitle>
              <CardDescription>Năm
                <select
